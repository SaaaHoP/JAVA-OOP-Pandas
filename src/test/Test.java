...
//        11) 지정한 열 인덱스 범위(begin<=, <end)의 서브테이블을 얻는다. (새 테이블)
        table.selectColumns(2, 5).print();
        anotherTable = table.selectColumns(2, 5);
...
//        table.sort(10, false, false).print();
...
//        20) 조건식을 만족하는 행을 얻는다.
//        (두번째 매개변수 조건을 만족하는 열이 골라지만, 그 열의 전체 행을 출력)
//        (나이가 소수점, Cabin이 null인 것들은 결과에서 빠지면 된다. 타입이 다르니까. 에러만 안나게 만들기)
